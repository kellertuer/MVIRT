...
MatrixXd mSPDGrad_X_D2(MatrixXd X, MatrixXd Y, MatrixXd Z) {
    ...
    JacobiSVD<MatrixXd> svd;
    EigenSolver<MatrixXd> eig;
    GeneralizedEigenSolver<MatrixXd> ges;// used for distance computations
    ArrayXXd Dist; // used for distance computations
    if (ItemSize != X.cols())
        return MatrixXd::Zero(1,1);
    ...
    R = mSPDLog(M,Y);
    //ges.compute(M,Y);
    Dist = ges.compute(M,Y).eigenvalues().transpose().cwiseAbs();
    Dist = log(Dist);    
    r = sqrt((Dist*Dist).sum());
    // Init G;
