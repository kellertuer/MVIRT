...
addRequired(ip,'x');
addRequired(ip,'eta');
addOptional(ip,'Gradient',[]);% if not set assume eta = grad
...
e_n = e-1;
while e > e_n
    x_n = vars.M.exp(vars.x,-vars.eta,stepsize);
    e_n = vars.F(x_n)+vars.c*stepsize*sum(...
        vars.M.dot(vars.x(vars.M.allDims{:},:),vars.eta(vars.M.allDims{:},:),Gradient(vars.M.allDims{:},:)));
